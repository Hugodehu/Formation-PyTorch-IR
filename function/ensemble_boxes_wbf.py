    ...
    for label in filtered_boxes:
        ...
        for j in range(0, len(boxes)):
            ...
            if index != -1:
                ...
                #get max of score at positon 1 in the list of new_boxes 
                max_score = max(np.array(new_boxes[index])[:, 1])
                #remove boxes with score lower than 0.15 if max_score is greater than 0.75
                if (max_score > .95):
                    idxs = np.argsort(np.array(new_boxes[index])[:, 1])
                    for idx in idxs: 
                        if new_boxes[index][idx][1] < 0.95:
                            new_boxes[index].pop(idx)
                            idx_max_boxes = np.argmax(np.array(new_boxes[index])[:, 1])
                            new_boxes[index].append(new_boxes[index][idx_max_boxes])
                            break
                elif(max_score > 0.75):
                    idxs = np.argsort(np.array(new_boxes[index])[:, 1])
                    for idx in idxs:
                        if new_boxes[index][idx][1] < 0.15:
                            new_boxes[index].pop(idx)
                            idx_max_boxes = np.argmax(np.array(new_boxes[index])[:, 1])
                            new_boxes[index].append(new_boxes[index][idx_max_boxes])
                            break
